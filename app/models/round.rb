class Round < ActiveRecord::Base
  ...
  def end_moving!
    ...
    GenericFleet.all.each do |fleet|
      Result.create(:generic_fleet_id => fleet.id, :planet => fleet.planet, :quantity => fleet.quantity, :generic_unit_id => fleet.generic_unit.id, :round => self, :squad => fleet.squad, :final_quantity => fleet.quantity, :producing_unit_id => fleet.producing_unit_id, :producing_unit2_id => fleet.producing_unit2_id, :fleet_name => fleet.fleet_name, :weapon1_id => fleet.weapon1_id, :weapon2_id => fleet.weapon2_id ) if fleet.planet.under_attack?
    end
